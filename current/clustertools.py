...
class Centroidstats(Clustering):
	...
	def cluster_predictors(self, vocab_used_for_feature_extraction):
		...
		try:
			self._centroid_check()
			centroiddicti=self._centroiddictmaker()
			if isinstance(vocab_used_for_feature_extraction, list):
...
def meanmachine(input_matrix, category_dict, feature_list, verbose, limit=100):
	"""
	...
	for num, feature in enumerate(feature_list):
		...
		for key, value in category_dict.items():
			...
			if tempi.shape[0] > limit:
				...
				if verbose=="csv":
					print "{},{},{},{},{},{},{},{},{},{}".format(feature, key, frame[0], mean*1000000, median*1000000, min*1000000, max*1000000, (max-min)*1000000, std*1000000, summ)
					print "stats test", scipy.stats.mannwhitneyu(featuretotal, featuretempi)
			else:
				skips.append(key)
