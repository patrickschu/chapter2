...
def dictmaker(folderlist, threshold=3000)
	...
	for folder in folderlist:
		...
		for fili in filis[:100]:
			inputfile=codecs.open(os.path.join(topdir, folder, fili), "r", "utf-8").read()
...
def clustermachine(matrix, algorithm, no_of_clusters=0):	
	centroids, labels=algorithm(wordmatrix_without_cat, len(clusters), minit='points')
	...
	(clusterlabel, index, [cat x, cat y],  
	
def categorymachine(folderlist):
	for folder in folders:
		catdicti={}
		filis=os.listdir(folder)
		for fili in filis[:100]:
...
def matrixmachine(folderlist, list_of_external_categories): 
	#constructing matrix
	wordmatrix=np.empty(shape=(1,len(featuredict)+list_of_external_categories))
	print "Matrix initial shape: ", np.shape(wordmatrix)
	# making a dictionary for the categories
	catdicti=categorymachine(folderlist)
	for folder in folderlist:
		filis=[i for i in os.listdir(path.join(topdir, folder)) if not i.startswith(".")]
		for fili in filis: 
			#establish category
			cat=catdicti[tagextractor(inputfile, "category1", fili)]
	...
	return (wordmatrix_without_cat=wordmatrix[len(external_categories):wordmatrix.shape[0],1:wordmatrix.shape[1]],
...
def main():
	matrixmachine()
	clustermachine(wordmatrix_without_cat,scipy.cluster.vq.kmeans2)
